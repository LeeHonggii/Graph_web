...
# 전역 변수로 RAG 설정 저장
rag_config = {"model": None, "retrieval_method": None, "is_configured": False}
...
async def process_pdf(file: UploadFile = File(...), config: str = Form(...)):
    try:
        import base64
        ...
        return JSONResponse(
            content={
                ...
                "result": {
                    ...
                    "embeddings_count": (
                        len(result["embeddings"]) if "embeddings" in result else 0
                    ),
                    "vector_store_status": result.get("vector_store", {}).get(
                        "status", "unknown"
                    ),
                },
            }
...
async def chat(request: Request):
    try:
        ...
        if not query:
            return JSONResponse(
                content={"status": "error", "message": "Query is empty"}
            )
        ...
        if not search_results:
            return JSONResponse(
                content={
                    "status": "success",
                    "message": "관련 내용을 찾을 수 없습니다.",
                }
            )

        # 검색된 문서들을 컨텍스트로 조합
        context = "\n\n".join(
            [f"페이지 {r['metadata']['page']}: {r['text']}" for r in search_results]
        )

        if not rag_config["is_configured"]:
            return JSONResponse(
                content={"status": "success", "message": "검색 결과:\n" + context}
            )

        # RAGgraph의 get_answer 함수 사용
        ai_response = RAGgraph.get_answer(
            input_text=query, memo_text=context, model_name=rag_config["model"]
        )
...
async def load_vectors(request: Request):
    try:
        ...
        if not collection_name:
            return JSONResponse(
                status_code=400,
                content={"status": "error", "message": "Collection name is required"},
            )
        ...
        return JSONResponse(
            status_code=500,
            content={"status": "error", "message": f"Failed to load vectors: {str(e)}"},
        )
